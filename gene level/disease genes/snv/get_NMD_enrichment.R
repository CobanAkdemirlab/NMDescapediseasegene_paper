get_NMD_enrichment = function(rds_name,ADfilter = 'off',p_cutoff=0.9,filter_type){
  l <- readRDS(rds_name)
  if(ADfilter == 'on'){
    genes.mat <- NULL
  ...
  for (p in 1:length(l)) {
    if(filter_type == 'all'){
      ...
      p_indexes[[p]] <- (as.numeric(l[[p]]$can.pvalue) >= can_cut) & l[[p]]$is_canonical
  ...
  out_name = paste(gsub('.rds','_',rds_name),'NMDesc_enriched_',filter_type,'.txt',sep = '')
