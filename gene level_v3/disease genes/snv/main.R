...
vcf_file = "/Users/jxu14/Desktop/NMDescapediseasegene_paper-main/new_NMDesc/data/clinvar/clinvar_20260201.vcf.gz"
...
##4.4 bootstrap and matched analysis
  ...
  # Gene-matched 配对Wilcoxon
  run_matched_ppi <- function(data, gene_set_label) {
    ...
    gene_summary <- data %>%
      filter(!is.na(variant_ppi_overlap)) %>%
      mutate(flag_val = as.integer(variant_ppi_overlap)) %>%
      group_by(ensembl_transcript_id, group) %>%
      summarise(prop_ppi = mean(flag_val), .groups = "drop")
    
    paired <- inner_join(
      gene_summary %>% filter(group == disease_grp) %>%
        rename(disease = prop_ppi) %>% select(ensembl_transcript_id, disease),
      gene_summary %>% filter(group == control_grp) %>%
        rename(control = prop_ppi) %>% select(ensembl_transcript_id, control),
      by = "ensembl_transcript_id"
    )
    
    message(sprintf("%s %s: %d个配对基因", gene_set_label,
                    if(disease_grp == "snv_disease") "SNV" else "FS", nrow(paired)))
    
    if (nrow(paired) < 2) return(NULL)
    
    if (all(paired$disease == paired$control)) {
      return(data.frame(gene_set = gene_set_label, model = "Gene-matched",
                        OR = 1, OR_low = NA, OR_high = NA, p_value = 1))
    }
    
    w_test <- wilcox.test(paired$disease, paired$control,
                          paired = TRUE, exact = FALSE)
    ratio  <- mean(paired$disease) / mean(paired$control)
    
    set.seed(42)
    boot_ratios <- replicate(1000, {
      disease_samp <- data %>%
        filter(group == disease_grp, !is.na(variant_ppi_overlap)) %>%
        group_by(ensembl_transcript_id) %>%
        slice_sample(n = 1) %>%
        ungroup() %>%
        select(ensembl_transcript_id, disease = variant_ppi_overlap)
      
      control_samp <- data %>%
        filter(group == control_grp, !is.na(variant_ppi_overlap)) %>%
        group_by(ensembl_transcript_id) %>%
        slice_sample(n = 1) %>%
        ungroup() %>%
        select(ensembl_transcript_id, control = variant_ppi_overlap)
      
      paired_b <- inner_join(disease_samp, control_samp, by = "ensembl_transcript_id")
      if (nrow(paired_b) < 2) return(NA)
      mean(paired_b$disease) / mean(paired_b$control)
    })
  ...
  run_all <- function(data, source_label) {
    ...
    bind_rows(
      ...
      run_matched_ppi(filter(data, gene_set == "SNV"),    "SNV"),
      run_matched_ppi(filter(data, gene_set == "FS"),     "FS")
    ) %>%
  ...
  variants_all4_PDB <- variants_motif(
    ...
    mart          = ensembl,
    touni_path    = "/Users/jxu14/Desktop/NMDescapediseasegene_paper-main/new_NMDesc/data/others/NIHMS1818854-supplement-2(A).csv",
    motif_path    = "/Users/jxu14/Desktop/NMDescapediseasegene_paper-main/new_NMDesc/data/others/NIHMS1818854-supplement-2(B).csv",
    lcs_path      = "/Users/jxu14/Desktop/NMDescapediseasegene_paper-main/new_NMDesc/data/others/Copy of NIHMS1818854-supplement-2.xls"
  )
...
#match by gene
   ...
   # ══════════════════════════════════════════════════════════════════════════════
   # PART 2 — GENE-MATCHED (paired Wilcoxon on proportions + continuous)
   # ══════════════════════════════════════════════════════════════════════════════
   run_matched <- function(data, gene_set_label, skip_flags = character(0)) {
     
     active_flags <- setdiff(flag_cols, skip_flags)
     disease_grp  <- if (gene_set_label == "SNV") "snv_disease" else "fs_disease"
     control_grp  <- if (gene_set_label == "SNV") "snv_control" else "fs_control"
     
     # helper: for each gene, randomly pick one variant per group, return paired summary
     sample_one_per_gene <- function(df, value_col, is_binary = FALSE) {
       disease_samp <- df %>%
         ...
         select(ensembl_transcript_id, disease)
       ...
       control_samp <- df %>%
         ...
         select(ensembl_transcript_id, control)
     ...
     flag_res <- lapply(active_flags, function(col) {
       ...
       df <- data %>% mutate(flag_val = as.integer(.data[[col]]))
       
       # observed: mean proportion per gene (all variants), then ratio
       gene_summary <- df %>%
         filter(!is.na(flag_val)) %>%
         group_by(ensembl_transcript_id, group) %>%
         summarise(prop = mean(flag_val), .groups = "drop")
       
       paired_obs <- inner_join(
         gene_summary %>% filter(group == disease_grp) %>% rename(disease = prop) %>% select(-group),
         gene_summary %>% filter(group == control_grp) %>% rename(control = prop) %>% select(-group),
         by = "ensembl_transcript_id"
       )
       
       if (nrow(paired_obs) < 2) {
         message(sprintf("Skipping %s in %s: too few paired transcripts", col, gene_set_label))
         return(NULL)
       }
       
       if (all(paired_obs$disease == paired_obs$control)) {
         return(data.frame(flag = col, OR = 1, OR_low = NA, OR_high = NA,
                           p_value = 1, row.names = NULL))
       }
       
       # observed test on mean proportions
       w_test <- wilcox.test(paired_obs$disease, paired_obs$control,
                             paired = TRUE, exact = FALSE)
       ratio  <- mean(paired_obs$disease) / mean(paired_obs$control)
       
       # bootstrap: each iteration pick one variant per gene per group
       set.seed(42)
       boot_ratios <- replicate(1000, {
         paired_b <- sample_one_per_gene(
           df %>% filter(!is.na(flag_val)) %>% rename_with(~ "flag_val", all_of("flag_val")),
           value_col = "flag_val"
         )
         if (nrow(paired_b) < 2) return(NA)
         mean(paired_b$disease) / mean(paired_b$control)
       ...
       data.frame(flag = col, OR = ratio, OR_low = or_low, OR_high = or_high,
                  p_value = w_test$p.value, row.names = NULL)
     }) %>% bind_rows()
     
     # ── Continuous: dist_to_cds_end_log ──────────────────────────────────────────
     df_cont <- data %>% filter(!is.na(dist_to_cds_end_log), is.finite(dist_to_cds_end_log))
     
     # observed: mean per gene per group
     gene_cont <- df_cont %>%
       group_by(ensembl_transcript_id, group) %>%
       summarise(mean_val = mean(dist_to_cds_end_log), .groups = "drop")
     
     paired_cont <- inner_join(
       gene_cont %>% filter(group == disease_grp) %>% rename(disease = mean_val) %>% select(-group),
       gene_cont %>% filter(group == control_grp) %>% rename(control = mean_val) %>% select(-group),
       by = "ensembl_transcript_id"
     )
     
     cont_res <- tryCatch({
       w_test <- wilcox.test(paired_cont$disease, paired_cont$control,
                             paired = TRUE, exact = FALSE)
       ratio  <- median(paired_cont$disease) / median(paired_cont$control)
       
       # bootstrap: pick one variant per gene per group each iteration
       set.seed(42)
       boot_ratios <- replicate(1000, {
         paired_b <- sample_one_per_gene(df_cont, value_col = "dist_to_cds_end_log")
         if (nrow(paired_b) < 2) return(NA)
         median(paired_b$disease) / median(paired_b$control)
       })
       or_low  <- quantile(boot_ratios, 0.025, na.rm = TRUE)
       or_high <- quantile(boot_ratios, 0.975, na.rm = TRUE)
       
       data.frame(flag = cont_col, OR = ratio, OR_low = or_low,
                  OR_high = or_high, p_value = w_test$p.value)
     }, error = function(e) {
       message(sprintf("Wilcoxon failed in %s: %s", gene_set_label, e$message))
       NULL
     })
     
     bind_rows(flag_res, cont_res) %>%
       mutate(gene_set = gene_set_label, model = "Gene-matched")
   }
   ...
   matched_snv <- run_matched(filter(variants_all5, gene_set == "SNV"),    "SNV", skip_snv)
   matched_fs  <- run_matched(filter(variants_all5, gene_set == "FS"),     "FS",  skip_fs)
   
   # ══════════════════════════════════════════════════════════════════════════════
   # COMBINE & BH-ADJUST
   # ══════════════════════════════════════════════════════════════════════════════
   combined <- bind_rows(unadj_snv, unadj_fs, matched_snv, matched_fs) %>%
     group_by(gene_set, model) %>%
     mutate(
       p_adj = p.adjust(p_value, method = "BH"),
       sig   = case_when(
