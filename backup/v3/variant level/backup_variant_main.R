...
variant_pfam_ppi <- function(
  ...
  # Build BH-adjusted p-value bracket data frame for stat_pvalue_manual()
  .pval_df <- function(summary_df, prop_col, event_col, total_col,
    ...
    out %>%
      mutate(
        ...
        label = paste0("FDR = ", signif(p_adj, 3))
      )
  }
  
  # Wilcoxon tests with BH correction across the planned comparisons in one plot
  .wilcox_pval_df <- function(data, value_col, pairs) {
    vals   <- data[[value_col]]
    ymax   <- max(vals, na.rm = TRUE)
    yrange <- diff(range(vals, na.rm = TRUE))
    if (!is.finite(yrange) || yrange == 0) yrange <- max(abs(ymax), 1)
    
    rows <- lapply(seq_along(pairs), function(i) {
      g1 <- pairs[[i]][1]; g2 <- pairs[[i]][2]
      x <- data[[value_col]][data$group == g1]
      y <- data[[value_col]][data$group == g2]
      ...
      data.frame(
        group1     = g1,
        group2     = g2,
        y.position = ymax + yrange * (0.10 + 0.12 * (i - 1)),
  ...
  ppi_dist_dat <- dat %>%
    filter(!is.na(variant_ppi_dist_to_nearest_interface_bp))
  pv_ppi_dist <- .wilcox_pval_df(
    ppi_dist_dat, "variant_ppi_dist_to_nearest_interface_bp", comparisons
  )
  ...
  p_ppi_dist <- ggplot(
    ...
    scale_fill_manual(values = group_colors, guide = "none") +
    stat_pvalue_manual(pv_ppi_dist, label = "label", xmin = "group1", xmax = "group2",
                       y.position = "y.position", tip.length = 0.01) +
  ...
  pv_ppi_log <- .wilcox_pval_df(
    ppi_log_dat, "variant_ppi_dist_to_nearest_interface_bp", comparisons
  )
  ...
  p_ppi_log <- ggplot(
    ...
    scale_y_log10() +
    scale_fill_manual(values = group_colors, guide = "none") +
    stat_pvalue_manual(pv_ppi_log, label = "label", xmin = "group1", xmax = "group2",
                       y.position = "y.position", tip.length = 0.01) +
  ...
  pv_pfam_dist <- .wilcox_pval_df(
    pfam_dist_dat, "dist_ptc_to_max_pfam_end_aa", comparisons
  )
  ...
  p_pfam_dist <- ggplot(
    ...
    geom_boxplot(width = 0.12, outlier.size = 0.6, color = "black") +
    scale_fill_manual(values = group_colors, guide = "none") +
    stat_pvalue_manual(pv_pfam_dist, label = "label", xmin = "group1", xmax = "group2",
                       y.position = "y.position", tip.length = 0.01) +
  ...
  pv_pfam_abs <- .wilcox_pval_df(pfam_abs_dat, "abs_dist", comparisons)
  ...
  p_pfam_abs <- ggplot(
    ...
    geom_boxplot(width = 0.12, outlier.size = 0.6, color = "black") +
    scale_fill_manual(values = group_colors, guide = "none") +
    stat_pvalue_manual(pv_pfam_abs, label = "label", xmin = "group1", xmax = "group2",
                       y.position = "y.position", tip.length = 0.01) +
...
# BH-adjust p-values within (gene_set, model) strata and assign sig stars
...
# Example usage:
# models <- run_dist_sanity_models(variants_all2)
# lapply(models, tidy, exponentiate = TRUE, conf.int = TRUE)

# ==============================================================================
# 4.2  MIXED-EFFECT MODEL
# ------------------------------------------------------------------------------
# Placeholder: account for non-independence of variants clustered within genes
# (and/or families) by fitting is_disease ~ flag + (1 | ensembl_transcript_id)
# style models, e.g. via lme4::glmer or glmmTMB.
# ==============================================================================

run_mixed_effect_flag_analysis <- function(variants_all5, flag_cols = FLAG_COLS) {
  library(lme4)
  map_dfr(flag_cols, function(flag) {
    ...
    fit <- tryCatch(
      glmer(as.formula(paste("is_disease ~", flag, "+ (1 | ensembl_transcript_id)")),
            data = df, family = binomial),
      error = function(e) { message(sprintf("Mixed model failed for %s: %s", flag, e$message)); NULL }
    )
    if (is.null(fit)) return(NULL)
...
tidy_mixed_results <- function(mixed_results, labels = FLAG_LABELS) {
  mixed_results %>%
    ...
    add_fdr_sig(group_vars = "model") %>%
    mutate(flag = ifelse(flag %in% names(labels), labels[flag], flag))
}

plot_mixed_effect_flags <- function(mixed_results) {
  plot_or_forest(
    tidy_mixed_results(mixed_results),
    facet_var = NULL, shape_var = NULL,
    title    = "Mixed-effect model: disease association per feature",
    subtitle = "Logistic GLMM, random intercept per transcript (OR, 95% CI)"
  )
}
...
# 4.3  HIERARCHICAL BAYESIAN MODEL
# ------------------------------------------------------------------------------
# Placeholder: partial-pooling of flag effects across gene sets (SNV/FS) and/or
# genes, e.g. via brms with a hierarchical prior on gene- or gene-set-level
# intercepts/slopes.
# ==============================================================================
...
plot_bayesian_by_geneset <- function(fit_lists) {
  plot_or_forest(
    ...
    subtitle = "Posterior OR with 95% credible interval; SNV and FS modeled separately"
...
plot_gene_matched_flags <- function(combined, labels = FLAG_LABELS) {
  ...
  plot_or_forest(
    ...
    subtitle = "Unadjusted logistic regression vs. gene-matched paired Wilcoxon"
  )
}
...
# ------------------------------------------------------------------------------
# Uncomment to run end-to-end once upstream objects (variants_all2, human_1_,
# variants_all5_1_0621) are available in the environment.
# ==============================================================================
...
write.csv(variants_all5, "variants_all0713.csv", row.names = FALSE)
