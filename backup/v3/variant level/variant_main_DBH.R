...
variants_motif <- function(variants_all2,
  ...
  variants_all2 <- variants_all2 %>%
    left_join(
      motif_max %>% 
        select(uniprot, `Protein Features`, Domains, SLiMs, MORFs, PTMs, NLSs),
      by = "uniprot"
    ) %>%
    mutate(
      variant_protein_flag = `Protein Features` * 3 >= cds_mutation_loc,
      variant_domain_flag  = Domains * 3 >= cds_mutation_loc,
      variant_slim_flag    = SLiMs * 3 >= cds_mutation_loc,
      variant_morf_flag    = MORFs * 3 >= cds_mutation_loc,
      variant_ptm_flag     = PTMs * 3 >= cds_mutation_loc,
      variant_nls_flag     = NLSs * 3 >= cds_mutation_loc
    )
  
  variants_all2 <- variants_all2 %>%
    left_join(
      LCS_max %>% select(uniprot, LCSs),
      by = "uniprot"
    ) %>%
    mutate(
      variant_LCS_flag = LCSs * 3 >= cds_mutation_loc
    )
