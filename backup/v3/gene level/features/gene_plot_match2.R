...
match_log <- read_csv("matching_log.csv", show_col_types = FALSE)
...
# Optional: inspect NA in all columns
...
feature_map <- tribble(
  ...
  "gene_ptm_flag",       "Post-Translational Modification Enrichment","C",
  "gene_LCS_flag",       "Low Complexity Sequence Enrichment",        "D"
) %>%
...
get_mcnemar_p <- function(data, feature, match_type_now, exact_cutoff = 25) {
  study_col <- paste0(feature, "_study")
  ctrl_col  <- paste0(feature, "_ctrl")
  ...
  b <- unname(tab["0", "1"])  # control only
  c <- unname(tab["1", "0"])  # study only
  discordant_n <- b + c
...
ordered_panels <- c("A", "B", "C", "D")
ordered_panels <- ordered_panels[ordered_panels %in% names(plot_list)]

combined_fig <- wrap_plots(plot_list[ordered_panels], ncol = 2) +
  plot_annotation(
...
# ── 14. Save outputs ──────────────────────────────────────────────────────────
ggsave("Matched_Gene_Level_Enrichment_4panels.pdf", combined_fig, width = 12, height = 10, dpi = 300)
ggsave("Matched_Gene_Level_Enrichment_4panels.png", combined_fig, width = 12, height = 10, dpi = 300)
write_csv(summary_table, "Matched_Gene_Level_Enrichment_4panels_summary.csv")

cat("\nDone!\n")
cat("Output files:\n")
cat("  Matched_Gene_Level_Enrichment_4panels.pdf\n")
cat("  Matched_Gene_Level_Enrichment_4panels.png\n")
cat("  Matched_Gene_Level_Enrichment_4panels_summary.csv\n")