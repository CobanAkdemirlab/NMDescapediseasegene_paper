get_NMD_enrichment = function(rds_name,ADfilter = 'off',p_cutoff=0.15,filter_type){
  l <- readRDS(rds_name)
  if(ADfilter == 'on'){
    genes.mat <- NULL
    genes.mat$gene = data.frame('gene' = (sapply(l, function(x) x$hgnc_symbol)))
    ...
    l = l[vapply(l, function(x) !is.null(x$hgnc_symbol) && !is.na(x$hgnc_symbol) && x$hgnc_symbol %in% AD_gene, logical(1))]
  }
  ...
  # ✅ 新增：将校正后的 p 值写回列表
  for (p in 1:length(l)) {
    l[[p]]$can.pvalue.adj <- can.pvalues.adj[p]
  }
  
  # 原有代码（改用校正后的值来计算分位数）
  can_cut <- quantile(unlist(can.pvalues.adj), p_cutoff, na.rm = TRUE)
  cat('can_cut (FDR-adjusted):', can_cut)
  
  
  
  for (p in 1:length(l)) {
    if(filter_type == 'all'){
      p_indexes[[p]] <- (as.numeric(l[[p]]$css.pvalue)>= css_cut | as.numeric(l[[p]]$can.pvalue)>= can_cut | as.numeric(l[[p]]$long.pvalue)>=long_cut) & l[[p]]$is_canonical
      ...
      p_indexes[[p]] <- (as.numeric(l[[p]]$can.pvalue.adj) <= can_cut) & l[[p]]$is_canonical
  ...
  out_name = paste(gsub('.rds','_',rds_name),'NMDesc_wald_enriched_',filter_type,'.txt',sep = '')
  writeLines(genes,out_name)
}