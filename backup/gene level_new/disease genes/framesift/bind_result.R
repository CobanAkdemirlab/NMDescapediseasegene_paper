...
##get the length of NMDesc variant region, count of variants that is NMDesc, use binomial test to get the p-value
#calculate p value
plus1_results_df <- data.frame(
...
for(i in seq_along(plus1_NMD_result)){
  ...
  #binomial test
  cds.start = 1
  cds.end   = max(cds.info$cds_end[cds.info$ensembl_transcript_id == transcript])
  chrom     = cds.info$chromosome_name[cds.info$ensembl_transcript_id == transcript][1]
  #change this 
  NMDesc.start = PTC_info$can_region_start[which(PTC_info$transcript == transcript & PTC_info$type == "plus1")]
  NMDesc.end = PTC_info$can_region_end[which(PTC_info$transcript == transcript & PTC_info$type == "plus1")]
  NMDesc.count = get_syn_count(chrom, NMDesc.start, NMDesc.end)
  if(!is.null(plus1_NMD_result[[i]][["can_region"]]) &&  NMDesc.count > 0 && !is.na(plus1_NMD_result[[i]][["can_region"]]) && NMDesc_can_count >0 && plus1_NMD_result[[i]][["can_region"]]>0){
    #plus1_NMD_result[[i]][["can_p"]] = binom.test(NMDesc_can_count,plus1_NMD_result[[i]][["can_region"]],all.variant/tr_length[1],alternative='greater')$p.value
    plus1_NMD_result[[i]][["can_p"]] = binom.test(NMDesc_can_count,NMDesc.count,all.variant/get_syn_count(chrom, cds.start, cds.end),alternative='greater')$p.value
...
#save plus1_NMD_result to local
saveRDS(plus1_NMD_result, file = "plus1_NMD_result20260201.rds")
...
#calculate p value
  ...
  for(i in seq_along(plus2_NMD_result)){
    ...
    chrom     = cds.info$chromosome_name[cds.info$ensembl_transcript_id == transcript][1]
    #change this 
    NMDesc.start = PTC_info$can_region_start[which(PTC_info$transcript == transcript & PTC_info$type == "plus2")]
    NMDesc.end = PTC_info$can_region_end[which(PTC_info$transcript == transcript & PTC_info$type == "plus2")]
    NMDesc.count = get_syn_count(chrom, NMDesc.start, NMDesc.end)
    if(!is.null(plus2_NMD_result[[i]][["can_region"]]) &&  NMDesc.count > 0 && !is.na(plus2_NMD_result[[i]][["can_region"]]) && NMDesc_can_count >0 && plus2_NMD_result[[i]][["can_region"]]>0){
      #plus2_NMD_result[[i]][["can_p"]] = binom.test(NMDesc_can_count,plus2_NMD_result[[i]][["can_region"]],all.variant/tr_length[1],alternative='greater')$p.value
      plus2_NMD_result[[i]][["can_p"]] = binom.test(NMDesc_can_count,NMDesc.count,all.variant/get_syn_count(chrom, cds.start, cds.end),alternative='greater')$p.value
    }else(plus2_NMD_result[[i]][["can_p"]] = NA)
    
    plus2_results_df <- rbind(plus2_results_df, data.frame(
      transcript = transcript,
      NMDesc_can_count = NMDesc_can_count,
      can_region = plus2_NMD_result[[i]][["can_region"]],
      all_variant = all.variant,
      stringsAsFactors = FALSE
    ))
  }
...
write.csv(fs_can_AD_FDR0.05_gene, "fs_can_AD_FDR0.05_gene.csv", row.names = FALSE)
