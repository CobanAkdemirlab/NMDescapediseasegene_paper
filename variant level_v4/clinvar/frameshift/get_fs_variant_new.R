...
fs_res = read_rds('fs.rds') #res_plp_ptc = res[intersect(ptc_ind,plp_ind)]; fs = res_plp_ptc[union(del_ind,ins_ind)]; fs_filtered = fs[which(fs@elementMetadata@listData[["key"]] %in% v_ch$key),]; fs = fs_filtered; write_rds(fs, file = 'fs.rds')
